# chainflux_app.py — ChainFlux Narrative Blockchain (Flask version for Vercel)
...
def home():
    html = """
    <html><head><title>ChainFlux</title><style>
    body { font-family: sans-serif; padding: 20px; background: #f0f0f0; }
    .block { background: white; padding: 15px; margin: 10px 0; border-radius: 8px; box-shadow: 0 0 6px rgba(0,0,0,0.1); }
    </style></head><body>
    <h1>🧠 ChainFlux Explorer</h1>
    {% for block in chain %}
    <div class="block">
        <h2>Block #{{ block.index }}: {{ block.title }}</h2>
        <p><b>Timestamp:</b> {{ block.timestamp }}</p>
        <p><b>Narrative:</b> {{ block.narrative }}</p>
        <p><b>Linked Blocks:</b> {{ block.linked_blocks }}</p>
...
def add():
    data = request.json
    if not all(k in data for k in ('title', 'narrative', 'linked_blocks')):
...
def mine():
    ...
    return jsonify({'message': 'No events to mine'})
